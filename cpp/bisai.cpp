  //设计一个支持大数运算的计算器，其中乘法使用分治法求解。该计算器支持加减乘除还有开运算。
  #include <iostream>
  #include <list>
  #include <string>
  #include <cstdio>
  ...
  list<char> Add(list<char> num1,list<char> num2)  //加法运算
  {
      ...
      else                                          //如果都为正
      {
          ...
          iter1--;
          iter2--;
          for(;(iter1 != num1.begin()) && (iter2 != num2.begin()); -r1,--iter2)  //进行运算
          {
              i = (*iter1 - '0') + (*iter2 - '0') + carry;
              //cout << (*iter1 - '0') << " " << (*iter2 - '0') << " " << iendl;
              ans.push_front((i % 10) + '0');
              carry = i / 10;
          }
          i = (*iter1 - '0') + (*iter2 - '0') + carry;
          //cout << (*iter1 - '0') << " " << (*iter2 - '0') << " " << i <dl;
          ans.push_front((i % 10) + '0');
          carry = i / 10;
          if(carry)
  ...
  list<char> Sub(list<char> num1,list<char> num2)                    //减法
  {
      list<char> ans;
      int sign = 0;
      list<char>::iterator iter1,iter2;
      ...
      else if( (*iter1) == '-' && (*iter2) != '-')              //如果一负一
      {
          num1.pop_front();
          ans = Add(num1,num2);
          ans.push_front('-');
      }
      else if( (*iter1) != '-' && (*iter2) == '-')              //如果一正一
      {
          num2.pop_front();
          ans = Add(num1,num2);
  ...
  list<char> Mul(list<char> num1,list<char> num2)  // 分治法求两大数的积
  {
      ...
      //print(num1);cout << endl;
      //print(num2);cout << endl;
      if(num1.size() == 1 && num2.size() == 1)     //如果两数都已是一位数，则运算
      {
          th = *(num1.begin()) - '0';
          tl = *(num2.begin()) - '0';
          th *= tl;
      ...
      else if(num1.size() == 1 && num2.size() > 1)            //如果num1位于1，则对Num1分治求积
      {
           if(*(num2.begin()) == '-')
             {
                  sign = 1;
           ...
           else
           {
              ...
              len = low.size();
              anshigh = Mul(num1,high);                 //num1分为两部分，hlow
              anslow = Mul(num1,low);
              for(i = 0; i < len; i++)
                  anshigh.push_back('0');
      ...
      else if(num2.size() == 1 && num1.size() > 1)              //如果num2位于1，则对Num2分治求积
      {
           if(*(num1.begin()) == '-')
             {
                  sign = 1;
           ...
           else
           {
              ...
              len = low.size();
              anshigh = Mul(num2,high);                   //num2分为两部分，hlow
              anslow = Mul(num2,low);
              for(i = 0; i < len; i++)
                  anshigh.push_back('0');
              ans = Add(anshigh,anslow);                    //合并结果
      ...
      else
      {
          ...
          if((flag1 == 1 && flag2 == 0)||(flag1 == 0 && flag2 == 1))  //如一正一负，则标记结果为负
          {
              sign = 1;
          }
          len1 = num1.size();
          len2 = num2.size();
          if(len1 == 1 || len2 == 1)                 //如果有一个数是一位数，则递归调用
          {
              ans = Mul(num1,num2);
              if(sign)
                  ans.push_front('-');
          ...
          {                                                //否则分治法求
              ...
              AC = Mul(num1high,num2high);                  //运用X=A*10^a + B C*10^b + D;
              AD = Mul(num1high,num2low);                   // X*Y = AC * 10 ^) + AD *10^a + BC * 10 ^b + BD公式求
  ...
  void Div(list<char> num1,list<char> num2)                 //用辗转相除求结
  {
      list<char> ans;
      list<char> temp;
      int len1,len2,len;
  ...
  void  Root(list<char> num1)                     //求大数的平方根，用枚举算法暴索算法求
  {
      list<char> ans;
      int len1;
      ...
      while(true)                                  //搜到则结束
      {
          temp = Mul(ans,ans);
          temp = Sub(num1,temp);
          if(*(temp.begin()) == '-')                      //如果当前ans的平方num1，则得结果
              break;
          ans = Add(ans,digit);
      }
      ans = Sub(ans,digit);
