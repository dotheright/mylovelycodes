...
while True:  #用来重复接收新的链接
    ...
    conn.sendall(u'欢迎致电 10086，请输入1xxx,0转人工服务.')
    #收消息
    while True: #用来基于一个链接重复收发消息
            try: #捕捉客户端异常关闭（ctrl+c）
                ...
                #发消息
                p=subprocess.Popen( str(recv_data),shell=True,stdout=subprocess.PIPE) #执行系统命令，windows平
                                                                                                      # 台命令的标准输出是gbk编码，需要转换
                ...
                print(str)
                send_data=bytes(str(res),encoding='utf8')
